...
```{r}
grim(pigs2$x, pigs2$n, percent = FALSE)
```

```{r}
grim(pigs1$x, pigs1$n, percent = FALSE)
```


```{r}
grim("5.19", 20, 1, show_rec = TRUE, rounding = "up_and_down")
```
```{r}
round_anti_trunc(-5.4924723, 3)
```


```{r}
reconstruct_rounded_numbers_scalar(2.9876, 3, rounding = "up_or_down_from", symmetric = FALSE)
```
...
```{r}
reconstruct_rounded_numbers(2.9856, 3, "anti_trunc", 5, TRUE)
```


```{r}
round_down_from(65.34845, 4, threshold = 5, symmetric=TRUE)
```
